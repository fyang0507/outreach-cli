...
`last_turn_ms_ago` is transcript-based, not raw audio energy, and is the better silence signal of the two — but its `null` means only "no data yet," not "the line just went idle."
