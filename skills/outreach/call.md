...
**Known voice-agent limitation:** an `audio_cleared` from a barge-in that interrupted an unusually long turn can be followed by on the order of 20-30 seconds where the agent generates nothing at all — no `runtime_error`, no new `speech` or `outbound_turn_generated` event, `activity.local.speaking` staying `false` throughout — even while the connection stays healthy and the remote side keeps talking. This is not a tool malfunction; it's a documented Gemini Live API behavior (interrupting a long response discards the ungenerated tail and truncates session context to what was actually delivered, and that truncation can leave the model unable to generate a reply for a while) — see [issue #118](https://github.com/fyang0507/outreach-cli/issues/118). The voice agent is instructed to keep any single turn under ~20 seconds specifically to avoid triggering this — if you still see it, wait it out (a fresh remote turn tends to unstick it) before assuming something broke.
