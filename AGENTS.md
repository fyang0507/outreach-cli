...
- An interrupt is the only producer of `audio_cleared`, and only when it actually had buffered audio to discard — an interrupt with nothing queued appends no `audio_cleared` and so can never produce a gap. If the next remote `speech` lands more than `TRANSCRIPTION_GAP_THRESHOLD_MS` (3s, an explicit provisional guess pending real barge-in timing) after that clear, the bridge writes a `transcription_gap` event (`speaker`, `gap_ms`) before that speech — one per clear, since the pending clear is consumed the moment it resolves. The gap is measured against whichever remote speech comes next even if a local turn happens first in between, because the point is whether the callee's own words made it into the transcript, not whether the agent replied. This is evidence-completeness input for a post-call read, not proof that anything was actually lost.
