...
- An interrupt is the only producer of `audio_cleared`, and only when it actually had buffered audio to discard — an interrupt with nothing queued appends no `audio_cleared` and so can never produce a gap. If the next remote `speech` lands more than `TRANSCRIPTION_GAP_THRESHOLD_MS` (6s) after that clear, the bridge writes a `transcription_gap` event (`speaker`, `gap_ms`) before that speech — one per clear, since the pending clear is consumed the moment it resolves. That threshold is measured, not guessed: across 31 `audio_cleared` → next-remote-`speech` intervals in 21 real calls, ordinary barge-in transcription lag already runs a few seconds (min 1574ms, p25 2265, median 3968), so a low threshold mostly fires on routine barge-ins; 6s sits at roughly the p75 mark and instead flags the genuinely large gaps (p90 9355, max 32246 — the 7.1s/11.4s class in `call_257cce0c2810.jsonl` that motivated this event). The gap is measured against whichever remote speech comes next even if a local turn happens first in between, because the point is whether the callee's own words made it into the transcript, not whether the agent replied. This is evidence-completeness input for a post-call read, not proof that anything was actually lost.
