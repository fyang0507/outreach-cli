...
- Runtime behavior lives at `<data_repo>/outreach/config.yaml`: operator identity, `voice_agent.default_persona`, `call.max_duration_seconds`, and the Gemini model, speech/voice, generation, VAD, turn-taking and transcription settings. Use `gemini-3.8-live`; `gemini.thinking` is no longer supported.
